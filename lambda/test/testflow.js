...
AWS.config.loadFromPath("./testflow_credentials.json");
...
const appId = "amzn1.ask.skill.5019def4-bc5f-4241-a8bc-9e115a03b9be";  // set this to match your skill's alexa.appId to remove warnings
const locale = 'es-ES';
...
function runTest(fileIndex) {
    if (fileIndex >= files.length) process.exit();
    ...
    const data = fs.readFileSync(fileName, "utf-8"); // open dialog sequence file and read Intents

    // set context
    request = {};
    lineArray = cleanArray(data.split("\n")); // remove empty or comment lines (# or //)
    current_line = 1;
...
const files = glob.sync(MyDialog);
console.log = OriginalConsoleLog;
